...
// -------------------------------

    {
        ...
        scriptCombo->set_active_text(getScriptToName().at(G_UNICODE_SCRIPT_INVALID_CODE));
...
// -------------------------------

    {
        ...
        rangeCombo->set_active_text(getRanges().at(4).second); // Basic Latin
...
void GlyphsPanel::glyphSelectionChanged()
{
    ...
    if (itemArray.empty()) {
        ...
        auto const &mappings = getScriptToName();
        if (auto const it = mappings.find(script); it != mappings.end()) {
...
void GlyphsPanel::rebuild()
{
    ...
    auto const &items = getScriptToName();
    ...
    if (active >= 0) {
        std::tie(lower, upper) = getRanges().at(active).first;
    }
    ...
    for (auto const it : present) {
        Gtk::ListStore::iterator row = store->append();
        Glib::ustring tmp(1, it);
        tmp = Glib::Markup::escape_text(tmp);  // Escape '&', '<', etc.
        (*row)[columns->code] = it;
        (*row)[columns->name]    = Glib::ustring::compose("<span font_desc=\"%1\">%2</span>",
                                                          fontspec, tmp);
        (*row)[columns->tooltip] = Glib::ustring::compose("<span font_desc=\"%1\" size=\"42000\">%2</span>",
                                                          fontspec, tmp);
    }
